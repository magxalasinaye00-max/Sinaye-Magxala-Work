<template>
  <div class="checkout-block">
    ...
    <form class="checkout-form">
      ...
      <p class="status info">Payment mode: Simulated (no real charge)</p>
      <p v-if="paymentError" class="status error">{{ paymentError }}</p>

      <div class="actions" v-if="cart.items.length > 0">
        <button type="button" class="btn-glow" :disabled="startingCheckout" @click="startSimulatedCheckout">
          {{ startingCheckout ? "Redirecting..." : "Complete Simulated Payment" }}
        </button>
      </div>
    </form>
  </div>
...
export default {
  ...
  mounted() {
    if (this.$route?.query?.canceled) {
      this.paymentError = "Checkout was canceled.";
    }
  },
  methods: {
    getApiBaseUrl() {
    ...
    async startSimulatedCheckout() {
      try {
        ...
        if (!response.ok || !data.url) {
          throw new Error(data.message || "Failed to start simulated checkout");
        }

        window.location.href = data.url;
      } catch (err) {
        this.paymentError = err.message || "Failed to start simulated checkout";
      } finally {
